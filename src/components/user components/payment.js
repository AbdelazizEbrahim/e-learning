...
const PaymentPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (email) {
                ...
                try {
                    // Fetch user profile
                    const profileResponse = await fetch(`/api/userProfile?email=${email}`);
    ...
    const handleDelete = async (courseCode) => {
        ...
        if (window.confirm("Are you sure you want to delete this course from your cart?")) {
            try {
                ...
                if (response.ok){
                    try {
                        const response = await fetch(`/api/enrollment?userEmail=${email}&courseCode=${courseCode}`, {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json',
                            },
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            // Initiate payment after enrollment process
            const paymentData = {
            ...
            if (paymentResult.status === 'success') {
                window.location.href = paymentResult.data.checkout_url; // Redirect to Chapa payment page
            } else {
